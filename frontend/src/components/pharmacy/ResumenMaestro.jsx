...
export default function ResumenMaestro() {
  ...
  const [almacen, setAlmacen] = useState(''); // Empty means all
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const res = await fetch(`${API_BASE}/pharmacy/master-outputs?${q}`, {
          headers: authHeaders()
        });
        if (!res.ok) throw new Error('Error al cargar datos del resumen maestro');
  ...
  const totalMonto = useMemo(() => filtered.reduce((s, r) => s + (Number(r.MontoTotal) || 0), 0), [filtered]);
  const totalCant = useMemo(() => filtered.reduce((s, r) => s + (Number(r.Cantidad) || 0), 0), [filtered]);

  const handleTopScroll = () => { if (topScrollRef.current && tableScrollRef.current) tableScrollRef.current.scrollLeft = topScrollRef.current.scrollLeft; };
  const handleTableScroll = () => { if (topScrollRef.current && tableScrollRef.current) topScrollRef.current.scrollLeft = tableScrollRef.current.scrollLeft; };
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2rem' }}>
        <div>
          <h1 style={{ fontSize: '1.875rem', fontWeight: 'bold', color: '#1E293B', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <span style={{ fontSize: '2.5rem' }}>📦</span> Resumen Maestro de Salidas
          </h1>
          <p style={{ color: '#64748b' }}>Consolidado de ventas POS, traspasos y cargos de hospitalización.</p>
        </div>
        ...
        {rows.length > 0 && (
          <button
            ...
            onClick={() => {
              const cols = [
                { header: 'Tipo Mov.', key: 'TipoMovimiento', align: 'center', width: 140 },
                { header: 'Folio/Factura', key: 'Folio', align: 'center', width: 100 },
                { header: 'Referencia/Paciente', key: 'Referencia', align: 'left', width: 220 },
                { header: 'Área / Destino', key: 'AreaDestino', align: 'center', width: 140 },
                { header: 'Código', key: 'Codigo', align: 'center', width: 100 },
                { header: 'Insumo / Medicamento', key: 'Insumo', align: 'left', width: 200 },
                { header: 'Cantidad', key: 'Cantidad', align: 'center', width: 80, type: 'num' },
                { header: 'Lote', key: 'Lote', align: 'center', width: 110 },
                { header: 'Caducidad', key: 'Caducidad', align: 'center', width: 100, type: 'date' },
                { header: 'Total ($)', key: 'MontoTotal', align: 'right', width: 100, type: 'money' },
                { header: 'Fecha', key: 'FechaMovimiento', align: 'center', width: 140, type: 'datetime' },
              ];
              const fmt = (val, col) => {
              ...
              const tMonto = filtered.reduce((s, r) => s + (Number(r.MontoTotal) || 0), 0);
              const tCant = filtered.reduce((s, r) => s + (Number(r.Cantidad) || 0), 0);
...
<table>
  ...
  <tr><td colspan="${cols.length}" class="subtitle-bar">Resumen Maestro de Salidas (POS, Traspasos, Cargos)</td></tr>
  <tr class="info-row"><td colspan="${cols.length}">Período: ${fechaDesde} al ${fechaHasta} &nbsp;|&nbsp; Almacén: ${almacen || 'TODOS'} &nbsp;|&nbsp; Registros: ${filtered.length} &nbsp;|&nbsp; Generado: ${fechaReporte}</td></tr>
  ...
  <tr class="total-row">
    ...
    <td colspan="1"></td>
              ...
              a.href = url; a.download = `Resumen_Maestro_Salidas_${fechaDesde}_${fechaHasta}.xls`;
      ...
      <div style={{ background: 'white', padding: '1.5rem', borderRadius: '12px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)', marginBottom: '2rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem' }}>
          ...
          <div>
            <label style={{ display: 'block', fontSize: '0.875rem', color: '#64748b', marginBottom: '0.25rem' }}>Almacén</label>
            <select value={almacen} onChange={e => setAlmacen(e.target.value)}
              ...
              <option value="FAR">Farmacia General (FAR)</option>
              <option value="CE">CEyE (CE)</option>
              <option value="URG">Urgencias (URG)</option>
              <option value="QUI">Quirófano (QUI)</option>
              <option value="HOS">Hospitalización (HOS)</option>
            </select>
          </div>
        </div>
      </div>
      ...
      {loading ? (
        <PremiumLoader text="Consolidando datos de salidas..." />
      ) : error ? (
        ...
        <div style={{ background: 'white', borderRadius: '16px', boxShadow: '0 4px 24px rgba(0,0,0,0.08)', overflow: 'hidden', border: '1px solid #e2e8f0' }}>
          ...
          <div style={{ 
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
              ...
              <div>
                <div style={{ fontSize: '0.65rem', textTransform: 'uppercase', color: '#94a3b8', letterSpacing: '0.05em', fontWeight: 600 }}>Piezas (Salidas)</div>
          ...
          <div ref={tableScrollRef} onScroll={handleTableScroll} style={{ overflowX: 'auto', maxHeight: '65vh' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '1300px' }}>
              <thead style={{ position: 'sticky', top: 0, zIndex: 10 }}>
                <tr>
                  <ColumnFilter columnKey="TipoMovimiento" data={rows} colFilters={colFilters} setColFilters={setColFilters} label="Tipo Mov." maxWidth="130px" />
                  <ColumnFilter columnKey="Folio" data={rows} colFilters={colFilters} setColFilters={setColFilters} label="Folio" maxWidth="90px" />
                  <ColumnFilter columnKey="Referencia" data={rows} colFilters={colFilters} setColFilters={setColFilters} label="Referencia" />
                  <ColumnFilter columnKey="AreaDestino" data={rows} colFilters={colFilters} setColFilters={setColFilters} label="Destino" maxWidth="130px" />
                  <ColumnFilter columnKey="Insumo" data={rows} colFilters={colFilters} setColFilters={setColFilters} label="Insumo" maxWidth="200px" />
                  <StaticTh label="Cant." />
                  <StaticTh label="Lote" />
                ...
                {paginated.length === 0 ? (
                  <tr>
                    <td colSpan="10" style={{ textAlign: 'center', padding: '4rem 2rem', color: '#94a3b8' }}>
                      ...
                      <div style={{ fontSize: '0.85rem' }}>Aún no hay datos para el periodo especificado o revisa los filtros.</div>
                    </td>
                  </tr>
                ) : paginated.map((r, i) => {
                  const caducado = r.Caducidad && new Date(r.Caducidad) < new Date();
                  return (
                    <tr key={i} className="cargos-row" style={{ borderBottom: '1px solid #f1f5f9', background: i % 2 === 0 ? '#ffffff' : '#f8fafc' }}>
                      <td style={{ padding: '12px 10px', textAlign: 'center' }}>
                         <span className="cargos-badge" style={{ 
                          background: r.TipoMovimiento === 'Hospitalización' ? 'linear-gradient(135deg, #e0e7ff, #c7d2fe)' : 'linear-gradient(135deg, #fef3c7, #fde68a)', 
                          color: r.TipoMovimiento === 'Hospitalización' ? '#3730a3' : '#92400e', 
                      ...
                      <td style={{ padding: '12px 10px', textAlign: 'center' }}>
                        <span style={{ fontWeight: '700', color: '#004687', fontSize: '0.82rem', background: '#EFF6FF', padding: '3px 10px', borderRadius: '6px', border: '1px solid #BFDBFE', display: 'inline-block' }}>{r.Folio}</span>
                      </td>
                      <td style={{ padding: '12px 10px' }}>
                        <div style={{ fontWeight: '600', color: '#0f172a', fontSize: '0.84rem' }}>{r.Referencia}</div>
                      </td>
                      <td style={{ padding: '12px 10px', textAlign: 'center' }}>
                        <span style={{ fontSize: '0.75rem', color: '#475569', fontWeight: '500' }}>{r.AreaDestino}</span>
                      </td>
                      <td style={{ padding: '12px 10px', maxWidth: '200px', whiteSpace: 'normal', wordWrap: 'break-word' }}>
                      ...
                      <td style={{ padding: '12px 10px', textAlign: 'center' }}>
                        <span style={{ fontWeight: '700', fontSize: '0.9rem', color: '#334155', background: '#f1f5f9', padding: '2px 10px', borderRadius: '4px' }}>{r.Cantidad}</span>
                      </td>
                      <td style={{ padding: '12px 10px', textAlign: 'center' }}>
                        <span style={{ fontWeight: '700', color: r.Lote ? '#b45309' : '#cbd5e1', fontSize: '0.82rem' }}>{r.Lote || '—'}</span>
                      </td>
                      <td style={{ padding: '12px 10px', textAlign: 'center' }}>
                        {r.Caducidad ? (
                          <span style={{ fontSize: '0.78rem', fontWeight: '500', background: caducado ? '#FEE2E2' : '#f0fdf4', color: caducado ? '#991b1b' : '#166534', padding: '2px 8px', borderRadius: '4px' }}>
          ...
          <div style={{ padding: '0.75rem 1.5rem', borderTop: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%)' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
              <button className="cargos-btn" onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
                style={{ padding: '6px 14px', border: '1px solid #cbd5e1', background: 'white', borderRadius: '8px', fontSize: '0.82rem', cursor: page === 1 ? 'not-allowed' : 'pointer', opacity: page === 1 ? 0.5 : 1, fontWeight: '600', color: '#334155' }}>← Anterior</button>
              {totalPages > 0 && <span style={{ padding: '6px 12px', background: '#004687', color: '#fff', borderRadius: '8px', fontSize: '0.8rem', fontWeight: '700', minWidth: '40px', textAlign: 'center' }}>{page} / {totalPages}</span>}
              <button className="cargos-btn" onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages || totalPages === 0}
                style={{ padding: '6px 14px', border: '1px solid #cbd5e1', background: 'white', borderRadius: '8px', fontSize: '0.82rem', cursor: (page === totalPages || totalPages === 0) ? 'not-allowed' : 'pointer', opacity: (page === totalPages || totalPages === 0) ? 0.5 : 1, fontWeight: '600', color: '#334155' }}>Siguiente →</button>
            </div>
          </div>
        </div>
      )}
