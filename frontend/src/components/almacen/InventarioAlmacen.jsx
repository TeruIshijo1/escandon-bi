...
export default function InventarioAlmacen() {
  ...
  const fetchInventory = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE}/almacen/inventario`, {
        headers: authHeaders()
      });
  ...
  // Filtrado local (SIN useMemo para evitar problemas de caché)
  const term = String(searchTerm || '').toLowerCase().trim();
  
  const filteredItems = (!term) ? items : items.filter(item => {
    if (!item) return false;
    const code = item.ItemCode ? String(item.ItemCode).toLowerCase() : '';
    const name = item.ItemName ? String(item.ItemName).toLowerCase() : '';
...
<table>
  ...
  <tr><td colspan="${cols.length}" class="subtitle-bar">Reporte: Sugerencias de Reabastecimiento - Almacén</td></tr>
  ...
  ${lowStockItems.map((row, i) => `<tr class="${i % 2 === 0 ? 'even' : 'odd'}">${cols.map(c => {
    ...
    link.href = url;
    link.setAttribute('download', `Sugerencias_Pedido_Almacen_${new Date().toISOString().slice(0,10)}.xls`);
    ...
    <div style={{ padding: '2rem', maxWidth: 'var(--content-max, 1400px)', margin: '0 auto', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ background: 'white', borderRadius: '12px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '1.25rem', borderBottom: '1px solid #e2e8f0', background: '#f8fafc', display: 'flex', gap: '1rem', flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', width: '100%', maxWidth: '600px', alignItems: 'center' }}>
            <input 
              type="text" 
            ...
            <tbody>
              {loading ? (
                ...
                filteredItems.map((item, idx) => {
                  ...
                  return (
                    <tr key={`row-${idx}`} style={{ borderBottom: '1px solid #e2e8f0', transition: 'background 0.2s' }}>
      ...
      {selectedItemHistory && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1001 }}>
          <div style={{ background: 'white', padding: '2rem', borderRadius: '12px', width: '90%', maxWidth: '900px', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem' }}>
              <div>
                <h2 style={{ margin: '0 0 0.5rem 0', color: '#0f172a' }}>Historial de Movimientos</h2>
            ...
            <div style={{ marginBottom: '1rem' }}>
              <input 
                type="text" 
                placeholder="🔍 Filtrar por destino o documento..." 
            ...
            {loadingHistory ? (
              <div style={{ padding: '2rem', textAlign: 'center', color: '#64748b' }}>Consultando movimientos en SAP...</div>
            ) : historyData.length === 0 ? (
              <div style={{ padding: '2rem', textAlign: 'center', color: '#64748b' }}>No hay traslados o salidas recientes registrados para este artículo.</div>
            ) : (
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                  <thead style={{ background: '#f8fafc', position: 'sticky', top: 0 }}>
                    <tr>
                      ...
                      <th style={{ padding: '0.75rem 1rem', borderBottom: '1px solid #e2e8f0', fontSize: '0.85rem', color: '#475569' }}>Documento</th>
                      <th style={{ padding: '0.75rem 1rem', borderBottom: '1px solid #e2e8f0', fontSize: '0.85rem', color: '#475569', textAlign: 'right' }}>Cant.</th>
                      <th style={{ padding: '0.75rem 1rem', borderBottom: '1px solid #e2e8f0', fontSize: '0.85rem', color: '#475569' }}>Almacén Origen</th>
                      <th style={{ padding: '0.75rem 1rem', borderBottom: '1px solid #e2e8f0', fontSize: '0.85rem', color: '#475569' }}>Destino / Comentarios</th>
                    </tr>
                  </thead>
                  <tbody>
                    {historyData.filter(d => {
                      ...
                      return !term || 
                             (d.TipoMovimiento && d.TipoMovimiento.toLowerCase().includes(term)) || 
                      ...
                      <tr key={i} style={{ borderBottom: '1px solid #e2e8f0' }}>
                        ...
                        <td style={{ padding: '0.75rem 1rem', fontWeight: '500', color: '#0f172a' }}>{'TR-' + h.Lote}</td>
                        <td style={{ padding: '0.75rem 1rem', textAlign: 'right', fontWeight: 'bold', color: '#b45309' }}>
                          {h.Cantidad < 0 ? h.Cantidad * -1 : h.Cantidad}
                        </td>
                        <td style={{ padding: '0.75rem 1rem', color: '#0f172a' }}>{h.Paciente || 'General'}</td>
                        <td style={{ padding: '0.75rem 1rem', color: '#475569', fontSize: '0.9rem' }}>
                          {h.TipoMovimiento || '-'}
                        </td>
                      </tr>
                    ))}
